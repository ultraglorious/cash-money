...
import { ActionIcon, Autocomplete, Badge, Group, NumberInput, Select, Table, TextInput } from "@mantine/core";
...
export function EditorRow({
  ...
  const commit = () => {
    ...
    const base = { accountId: accountId as Ulid, date: toIso(date), payee: payee.trim(), memo: memo.trim(), cleared: initial?.cleared ?? "cleared" as const };
    if (splits) {
      onSubmit({ ...base, amount: splitTotal, splits });
    } else {
      onSubmit({ ...base, amount: signedMagnitude(), ...(categoryId ? { categoryId: categoryId as Ulid } : {}) });
    }
  };
  ...
  return (
    <Table.Tr bg="var(--mantine-color-indigo-light)">
      <Table.Td>
        <DatePickerInput size="xs" value={date} onChange={setDate} valueFormat="DD MMM" popoverProps={{ withinPortal: true }} />
      </Table.Td>
      {!single && (
        <Table.Td>
          <Select size="xs" placeholder="Account" data={accountData} value={accountId} onChange={setAccountId} searchable comboboxProps={{ withinPortal: true }} />
        </Table.Td>
      )}
      <Table.Td>
